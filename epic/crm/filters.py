...
class TextFilter(admin.filters.FieldListFilter):
    def __init__(self, field, request, params, model, model_admin, field_path):
        self.lookup_kwarg = field_path

        self.default = self._get_default_values(request, model_admin, field_path)
    ...
    def _get_custom_title(request, model_admin, field_path):
        title_method_name = "get_{0}_title".format(field_path)
    ...
    def _get_default_values(request, model_admin, field_path):
        default_method_name = "get_{0}_default".format(field_path)
    ...
    def _get_form_class(self):
        ...
        form_class = type(str("TextFilter"), (forms.BaseForm,), {"base_fields": fields})
                ...
                (
                    ...
                    forms.CharField(
                        ...
                        widget=forms.TextInput(
                            attrs={"placeholder": _("Enter filter")}
                        ),
                        required=False,
                        localize=True,
                    ),
    ...
    def _make_query_filter(self, _request, validated_data):
        query_params = {}
        value = validated_data.get(self.lookup_kwarg, None)

        if value:
            query_params["{0}__contains".format(self.field_path)] = value
...
class NumericRangeFilter(admin.filters.FieldListFilter):
    ...
    def _get_form_class(self):
        ...
        form_class = type(
            str("NumericRangeFilter"), (forms.BaseForm,), {"base_fields": fields}
        )
